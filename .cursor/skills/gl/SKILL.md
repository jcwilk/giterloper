...
2. Add/subtract as needed, reconcile, then promote with `./.cursor/skills/gl/scripts/gl promote --pin <name>`.
...
`pin add` clones and indexes automatically. No separate clone/index steps needed.
...
- Run `./.cursor/skills/gl/scripts/gl diagnostic` to check pin health (clones, indexes, vectors, branch freshness).
...
- Confirm with the user before destructive actions (subtract, intersect).
