...
```bash
node .cursor/skills/gl/gl.mjs --help
```
...
1. Run `node .cursor/skills/gl/gl.mjs query "<question>"` or `node .cursor/skills/gl/gl.mjs search "<keywords>"`.
2. If needed, run `node .cursor/skills/gl/gl.mjs get "<path>" --full`.
3. Respond with citations to file paths/headings from retrieved content.

### Retrieving context

1. Run multiple `node .cursor/skills/gl/gl.mjs search` or `node .cursor/skills/gl/gl.mjs query` commands if the request spans topics.
2. Fetch full docs with `node .cursor/skills/gl/gl.mjs get`.
...
1. Queue new content (stdin): `echo "<markdown>" | node .cursor/skills/gl/gl.mjs add --pin <name> [--name <file>]`.
2. Repeat `add` as needed to build a paper trail of queued changes.
3. Reconcile queue into `knowledge/`: `node .cursor/skills/gl/gl.mjs reconcile --pin <name>`.
4. Optional finalization: `node .cursor/skills/gl/gl.mjs promote --pin <name>`.

### Subtracting knowledge

1. Queue subtraction intent (stdin): `echo "<markdown>" | node .cursor/skills/gl/gl.mjs subtract --pin <name> [--name <file>]`.
2. Reconcile queue into `knowledge/`: `node .cursor/skills/gl/gl.mjs reconcile --pin <name>`.

### Merging knowledge branches

1. Ensure both pins are branched.
2. Merge source pin branch into target pin branch: `node .cursor/skills/gl/gl.mjs merge <source-pin> <target-pin>`.
...
3. Promote with `node .cursor/skills/gl/gl.mjs promote <branch>`.

### Pin management

- List: `node .cursor/skills/gl/gl.mjs pin list`
- Add: `node .cursor/skills/gl/gl.mjs pin add <name> <source> [--ref <ref>] [--branch <branch>]`
- Remove: `node .cursor/skills/gl/gl.mjs pin remove <name>`
- Update SHA: `node .cursor/skills/gl/gl.mjs pin update <name> [--ref <ref>]`
...
- Another pin name (resolve via `node .cursor/skills/gl/gl.mjs pin list` and `--pin`)
...
- Prefer `node .cursor/skills/gl/gl.mjs status` before making assumptions about local state.
- Use `node .cursor/skills/gl/gl.mjs verify` after clone/index or promotions.
