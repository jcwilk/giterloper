---
name: gl
description: Interact with giterloper knowledge stores connected to this project. Use when the user needs to search, query, retrieve, check health, or manage pins in .giterloper/pinned.yaml.
---
...
Use this skill for giterloper operations in this project. These are the main commands only; extended/debugging commands exist in `gl extended` (not documented here).
...
Pin add materializes (clone and index) automatically.
...
- If the script reports a state error, fix state (pin add/update) before retrying.
