---
name: gl
description: Interact with giterloper knowledge stores connected to this project. Use when the user needs to search, query, retrieve, add, subtract, reconcile, promote, merge, or manage pins in .giterloper/pinned.yaml.
---
...
Use this skill for giterloper operations in this project. The main `gl` CLI exposes a minimal surface for the happy path; debugging commands live in `gl extended` (not documented here).
...
2. Use `subtract` to remove reference-overlapping content from the base, or for manual edits use `gl extended stage` (debug workflow).
3. Promote with `./.cursor/skills/gl/scripts/gl promote`.

### Pin management

- List: `./.cursor/skills/gl/scripts/gl pin list`
- Add: `./.cursor/skills/gl/scripts/gl pin add <name> <source> [--ref <ref>] [--branch <branch>]` (auto-clones and indexes)
...
- If diagnostic fails, fix state (pin add/update, or use `gl extended clone`/`gl extended index` for manual materialization).
- Write operations fail if the tracked branch is stale; run `./.cursor/skills/gl/scripts/gl pin update <name>` and retry.
- Confirm with the user before destructive actions (subtract, pin remove).
- Never edit `.giterloper/versions/` directly; write via add/subtract/reconcile/promote only.
