---
name: gl
description: Interact with giterloper knowledge stores connected to this project. Use when the user needs to search, query, retrieve, diagnose, add, subtract, reconcile, promote, merge, or manage pins in .giterloper/pinned.yaml.
---
...
2. Use subtract to remove non-overlapping content, then reconcile and promote.
3. For manual staging/editing workflows, see gl-extended in AGENTS.md.
...
- If diagnostic reports a state error, run `gl pin update <name>` and retry write ops. For deeper debugging, use gl-extended (see AGENTS.md); prefer fixing via main commands.
