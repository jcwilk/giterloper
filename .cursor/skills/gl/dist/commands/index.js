...
import { collectionName, pinQmd, collectionExists, contextExists, } from "../qmd.js";
import { writeLocalConfig } from "../config.js";
import { detectGpuMode, ensureGpuConfig } from "../gpu.js";
import { requirePinBranch, assertBranchReadyForWrite, ensureWorkingClone, assertBranchFresh, branchFreshSoft, } from "../branch.js";
import { clonePin, indexPin, teardownPinData, updatePinSha, removeStagedDir, } from "../pin-lifecycle.js";
import { info, commandOutput, parseFlag, consumeBooleanFlag, ensureHelpNotRequested, } from "../cli.js";
...
function cmdGpu(state, args) {
    ...
    if (rest.length > 0)
        fail("unexpected arguments: gl gpu [--cpu]", EXIT.USER);
    ...
    if (cpuFlag.found) {
        ...
        if (!state.globalJson)
            info("GPU mode set to CPU. Run `gl gpu` without --cpu to re-detect after installing CUDA.");
        return;
    }
...
function cmdPinList(state, args) {
    ...
    commandOutput(state.globalJson
        ? pins
        : pins.map((pin, idx) => {
...
function cmdPinAdd(state, args) {
    ...
    if (args.length < 2)
        fail("usage: gl pin add <name> <source> [--ref <ref>] [--branch <branch>]", EXIT.USER);
    const name = args[0];
    const source = args[1];
    ...
    if (rest.length > 0)
        fail(`unexpected arguments: ${rest.join(" ")}`, EXIT.USER);
...
function cmdPinRemove(state, args) {
    ...
    if (args.length !== 1)
        fail("usage: gl pin remove <name>", EXIT.USER);
    const name = args[0];
    const pins = readPins(state);
    const target = pins.find((p) => p.name === name);
    if (!target)
        fail(`pin "${name}" not found`, EXIT.USER);
...
function cmdPinUpdate(state, args) {
    ...
    if (args.length < 1)
        fail("usage: gl pin update <name> [--ref <ref>]", EXIT.USER);
    const name = args[0];
...
function cmdClone(state, args) {
    ...
    if (rest.length > 0)
        fail(`unexpected arguments: ${rest.join(" ")}`, EXIT.USER);
    if (allParsed.found && pinParsed.found)
        fail("use either --all or --pin, not both", EXIT.USER);
    const pins = allParsed.found ? readPins(state) : [resolvePin(state, pinParsed.found ? (pinParsed.value ?? undefined) : undefined)];
    for (const pin of pins)
        clonePin(state, pin, { branch: pin.branch });
...
function cmdTeardown(state, args) {
    ...
    if (args.length !== 1)
        fail("usage: gl teardown <name>", EXIT.USER);
...
function cmdSearchLike(state, mode, args) {
    ...
    if (args.length < 1)
        fail(`usage: gl ${mode} <${mode === "search" ? "query" : "question"}>`, EXIT.USER);
    let rest = [...args];
    const pinParsed = parseFlag(rest, "--pin");
    rest = pinParsed.args;
...
function cmdGet(state, args) {
    ...
    if (args.length < 1)
        fail("usage: gl get <path> [--pin <name>] [--full]", EXIT.USER);
    let rest = [...args];
    const pinParsed = parseFlag(rest, "--pin");
    rest = pinParsed.args;
    ...
    if (rest.length < 1)
        fail("missing path argument", EXIT.USER);
...
function cmdStage(state, args) {
    ...
    if (!branch)
        fail("usage: gl stage <branch> [--pin <name>]", EXIT.USER);
...
function cmdStageCleanup(state, args) {
    ...
    const branch = rest[0] || pin.branch;
    if (!branch)
        fail("usage: gl stage-cleanup <branch> [--pin <name>]", EXIT.USER);
    const dir = stagedDir(state, pin.name, branch);
...
function cmdVerify(state, args) {
    ...
    if (pins.length === 0)
        fail("no pins configured", EXIT.STATE);
    const results = [];
    for (const pin of pins) {
        const cdir = cloneDir(state, pin);
        const collection = collectionName(pin);
    ...
    if (!allOk)
        fail("verify: not all pins are healthy", EXIT.STATE);
}
...
function cmdMerge(state, args) {
    ...
    if (args.length !== 2)
        fail("usage: gl merge <source-pin> <target-pin>", EXIT.USER);
...
export function runCommand(state, cmd, args) {
    if (cmd === "status")
        return cmdStatus(state, args);
    if (cmd === "pin") {
        if (args.length === 0)
            fail("usage: gl pin <list|add|remove|update>", EXIT.USER);
        const [sub, ...subArgs] = args;
        if (sub === "list")
            return cmdPinList(state, subArgs);
        if (sub === "add")
            return cmdPinAdd(state, subArgs);
        if (sub === "remove")
            return cmdPinRemove(state, subArgs);
        if (sub === "update")
            return cmdPinUpdate(state, subArgs);
        fail(`unknown pin subcommand "${sub}"`, EXIT.USER);
    }
    if (cmd === "gpu")
        return cmdGpu(state, args);
    if (cmd === "clone")
        return cmdClone(state, args);
    if (cmd === "index")
        return cmdIndex(state, args);
    if (cmd === "teardown")
        return cmdTeardown(state, args);
    if (cmd === "search")
        return cmdSearchLike(state, "search", args);
    if (cmd === "query")
        return cmdSearchLike(state, "query", args);
    if (cmd === "get")
        return cmdGet(state, args);
    if (cmd === "stage")
        return cmdStage(state, args);
    if (cmd === "promote")
        return cmdPromote(state, args);
    if (cmd === "stage-cleanup")
        return cmdStageCleanup(state, args);
    if (cmd === "add")
        return cmdAddLike(state, args, "add");
    if (cmd === "subtract")
        return cmdAddLike(state, args, "subtract");
    if (cmd === "reconcile")
        return cmdReconcile(state, args);
    if (cmd === "merge")
        return cmdMerge(state, args);
    if (cmd === "verify")
        return cmdVerify(state, args);
