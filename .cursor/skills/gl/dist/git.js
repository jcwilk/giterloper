...
export function resolveSha(source, ref = "HEAD") {
...
export function resolveBranchSha(source, branch) {
    const remote = toRemoteUrl(source);
...
export function resolveBranchShaSoft(source, branch) {
    const remote = toRemoteUrl(source);
    const out = runSoft("git", ["ls-remote", "--heads", remote, branch]);
    if (!out.ok || !out.stdout)
        return null;
...
export function setCloneIdentity(dir) {
...
export function verifyCloneAtSha(pin, clonePath) {
