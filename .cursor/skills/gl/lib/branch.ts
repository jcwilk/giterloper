/**
 * Branch operations: require branch, working clone, freshness.
 */
import { existsSync, rmSync } from "node:fs";
import path from "node:path";
import { EXIT, fail } from "./errors.js";
import { run, runSoft, isBranchNotFoundError } from "./run.js";
  ...
  if (!existsSync(dir)) {
    ...
    if (!result.ok) {
      if (isBranchNotFoundError(result)) {
        ...
        infoFn(`branch "${pin.branch}" not found; creating from default branch`);
...
export function assertBranchFresh(state: GlState, pin: Pin, workingDir: string): void {
  if (!pin.branch) return;
...
export interface BranchFreshSoftResult {
  fresh: boolean | null;
  localSha: string | null;
  remoteSha: string | null;
}

export function branchFreshSoft(state: GlState, pin: Pin): BranchFreshSoftResult {
