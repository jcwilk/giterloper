/**
 * Pin lifecycle: clone, index, teardown, update SHA, remove staged.
 */
  ...
  if (branch) {
    ...
    if (!result.ok) {
      if (isBranchNotFoundError(result)) {
        if (existsSync(cdir)) rmSync(cdir, { recursive: true, force: true });
        infoFn(`branch "${branch}" not found; cloning from ${fallbackRef || "default"} and checking out ${pin.sha}`);
...
export function indexPin(
  ...
  if (!collectionExists(pin, collection)) {
    ...
    infoFn(`collection ${collection} already exists`);
  }
  ...
  if (needsEmbed === 0) {
    infoFn(`collection ${collection} already fully embedded, skipping qmd embed`);
  } else {
...
export function updatePinSha(
  ...
  clonePin(state, newPin, { branch: cloneBranch }, infoFn);
  ensureGpuConfig(state, infoFn);
  indexPin(state, newPin, infoFn);
