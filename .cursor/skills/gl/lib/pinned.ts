/**
 * Pinned.yaml parsing, serialization, and I/O.
 */
...
export function parsePinned(content: string): Pin[] {
  const pins: Pin[] = [];
  let current: { name: string; source: string | null; sha: string | null; branch?: string } | null = null;
  ...
  for (const rawLine of lines) {
    ...
    if (indent === 0) {
      ...
      pins.push(current as Pin); // mutated by nested key lines
      continue;
    }
