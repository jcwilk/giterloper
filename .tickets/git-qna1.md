...
## Closure checklist (audit evidence)

- **Artifacts read:** root `Dockerfile`, `fly.toml`, `scripts/run-docker.sh`; spot-checked env usage in `lib/gl-mcp-server.ts`, `lib/mcp-session-store.ts`, `lib/gl-core.ts`; cross-checked HTTP MCP surface with `specs/MCP.md`.
- **Fixes:** `docs/DEPLOYMENT_REQUIREMENTS.md` and `docs/FLY_IO_DEPLOYMENT.md` — added mandate-aligned “operational only” framing + links to `specs/MCP.md` / mandate; corrected production image description (partial `COPY`, `unzip`, `/data` CWD); documented `KNOWLEDGE_STORE_REMOTE`; replaced stale generic Dockerfile sketch in Fly doc with repo-accurate summary; clarified local Docker port mapping; repaired internal links to `README.md`, `AGENTS.md`, `Dockerfile`, `fly.toml`, `HIERARCHICAL_TRUTH_AND_ALIGNMENT_MANDATE.md`.
- **Commands run:** `./scripts/check_all.sh` (typecheck + full manifest tests) — passed.
