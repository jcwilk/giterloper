...
- **Commits:** **Planning commits** = spec/mandate edits **and** new tickets **only**—**never** mixed with code/test implementation in the same commit (implementation closes in separate commits). When **file-tickets** is used to file alignment work (e.g. from the spec-change flow), it must treat **uncommitted changes under `specs/`** and the root **mandate** file (`HIERARCHICAL_TRUTH_AND_ALIGNMENT_MANDATE.md`) as **first-class context** alongside the invoking conversation (e.g. spec-change agent instructions); the **same** planning commit must **stage and record** those spec/mandate paths **together with** the new or updated `.tickets/*.md` produced from that context—no tickets-only commit that drops the spec delta that drove the filing. Spec-change subagent (**git-rm4l**) implements that bundle and runs verifier before completion (**git-incv**).
