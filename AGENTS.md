...
**`docs/`:** Deployment and operational notes may describe behavior incidentally but **must not lock** product truth. On conflict with `specs/*`, tests, or intentional product contracts, **update `docs/`** to conform. The MCP product contract is **[specs/MCP.md](./specs/MCP.md)** only (no duplicate stub at the repo root).
