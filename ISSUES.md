...
**Fix (applied):** `gl merge` now uses the GitHub REST API (`POST /repos/{owner}/{repo}/merges`) to perform the merge entirely on the server. No local fetch or clone is required; clones remain shallow. Requires `GITERLOPER_GH_TOKEN`. On 409 conflict, fails with instructions to resolve on GitHub (create PR, resolve in UI, merge), then `gl pin update`.

**Status:** Resolved. E2E test added in `gl-branching.test.ts`.
