...
**Was:** `gl merge` used `git fetch --depth 1` and local `git merge`, which fails with shallow clones (no merge base).

**Fix (applied):** Merge is done remotely via GitHub REST API `POST /repos/{owner}/{repo}/merges`. No local fetch or clone needed; clones stay shallow. Requires `GITERLOPER_GH_TOKEN`. On conflict (409), creates a PR for manual resolution.
