...
## 6. `gl merge` shallow fetch (resolved)

**Observed:** `gl merge` used `git fetch --depth 1` when fetching; with shallow history git could not find the merge base.

**Fix applied:** Deepen both target and source before merge: `git fetch origin <target.branch> --depth 100` and `git fetch <remote> <source.branch> --depth 100`. This gives git enough history to find the merge base for typical branch divergence. Conflict resolution flow unchanged: resolve in staged clone, then `gl promote --pin <target-pin>`.

**Future improvement:** A more robust approach would attempt trivial merges remotely first (e.g. via `gh`) without downloading history; for non-trivial cases, stage for manual merge and add `gl complete-merge`. Not implemented.
