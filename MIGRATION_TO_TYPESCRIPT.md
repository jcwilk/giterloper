...
- [x] **2.2** Add `tests/unit/reconcile.test.mjs`; use Node built-in test runner (`node --test`)
- [x] **2.3** Extract `lib/paths.ts`: findProjectRoot, ensureDir, cloneDir, stagedDir (no removeStagedDir yet)
- [x] **2.4** Add `tests/unit/paths.test.mjs` for findProjectRoot, cloneDir, stagedDir (use temp dirs)
- [x] **2.5** Update `gl.mjs` to import from dist/ and remove inlined implementations
- [x] **2.6** Run E2E: `node scripts/run-e2e.mjs` (unit tests pass; E2E require GITERLOPER_GH_TOKEN)
...
- [x] **4.3** Add `tests/unit/pinned.test.mjs` for parsePinned, serializePins (roundtrip, edge cases)
- [x] **4.4** Update `gl.mjs` to import from dist/
- [x] **4.5** Run E2E (unit tests pass)
