...
Or add pins with `gl pin add`, which clones and indexes automatically. Use `gl-extended` for clone/index when materializing from an existing `pinned.yaml`. If CUDA is missing but an NVIDIA GPU is present, run `gl-extended gpu --cpu` (with user approval).
