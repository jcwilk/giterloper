...
`gl-extended clone` fetches pinned stores into `.giterloper/versions/`. `gl-extended index` adds qmd collections and runs `qmd embed`. To add a new pin (clones and indexes automatically), use `gl pin add`. If CUDA is missing but an NVIDIA GPU is present, `gl` will exit with instructions; present the choice to the user before proceeding.
