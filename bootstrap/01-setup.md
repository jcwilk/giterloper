...
5. **CUDA Toolkit** (recommended for GPU acceleration):
   ...
   **STOP.** If `nvidia-smi` succeeds but `nvcc` fails, inform the user explicitly. Explain that missing CUDA causes slow qmd commands on every invocation. Do not proceed until the user chooses: (a) install the CUDA Toolkit, or (b) explicitly opt into CPU-only mode. Do not run `gl-extended gpu --cpu` without explicit user approval.
   ...
   - Ubuntu/Debian example:
     ...
     ```
   - After installing, run `./.cursor/skills/gl/scripts/gl-extended gpu` to re-detect. If the user prefers CPU-only, run `gl-extended gpu --cpu` (only after they have explicitly approved).
...
**Option A** — Add a new pin (auto-clones and indexes):
```sh
./.cursor/skills/gl/scripts/gl pin add <name> <source> [--ref <ref>] [--branch <branch>]
```

**Option B** — If pins already exist in `pinned.yaml`, clone and index manually:
```sh
./.cursor/skills/gl/scripts/gl-extended clone
./.cursor/skills/gl/scripts/gl-extended index
```

Clone fetches pinned stores into `.giterloper/versions/`. Index adds qmd collections and runs `qmd embed`. If CUDA is missing but an NVIDIA GPU is present, run `./.cursor/skills/gl/scripts/gl-extended gpu --cpu` (with user approval). See AGENTS.md for gl-extended usage.
