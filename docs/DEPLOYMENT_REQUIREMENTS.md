...
- **Env (spot-checked vs `lib/gl-mcp-server.ts`, `lib/mcp-session-store.ts`, `lib/gl-core.ts`):** `MCP_HOST`, `MCP_PORT`, `MCP_TOKEN` (Bearer), `MCP_INSECURE` (dev only), `MCP_SESSION_TTL_MS`; `GITERLOPER_GH_TOKEN` (optional, for git + GitHub API); **`KNOWLEDGE_STORE_REMOTE`** (**required** for normal MCP operation: non-empty valid Git remote; server exits at startup if missing/invalid). New sessions bootstrap `_session` from that remote at default branch HEAD before tools run (`specs/MCP.md`). For MCP test mode (session root **`.giterloper_test`**), use **`GITERLOPER_MCP_TEST_MODE`** plus **`TEST_KNOWLEDGE_STORE_REMOTE`** per `specs/MCP.md` and `tests/README.md`—not for production defaults.
