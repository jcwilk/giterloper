...
import { EXIT, GlError, fail } from "./lib/errors.js";
import {
...
import { isBranchNotFoundError, run, runSoft } from "./lib/run.js";
import {
...
import { resolveSha, setCloneIdentity, toRemoteUrl } from "./lib/git.js";
import { cloneDir, ensureDir, findProjectRoot, stagedDir } from "./lib/paths.js";
import {
...
import { detectGpuMode, ensureGpuConfig } from "./lib/gpu.js";
import {
  clonePin,
  indexPin,
  removeStagedDir,
...
import type { GlState } from "./lib/types.js";
...
function cmdGpu(state: GlState, args: string[]) {
  ...
  const out: { gpuMode: string; reason?: string } = { gpuMode: detected.mode };
  if (detected.mode === "cpu") out.reason = detected.reason;
  commandOutput(out, state.globalJson);
  if (!state.globalJson) {
    if (detected.mode === "cuda") {
      info("CUDA detected. GPU acceleration enabled.");
    } else if (detected.mode === "cpu") {
      const reason = (detected as { reason?: string }).reason;
      if (reason === "no-gpu") {
...
function cmdPinAdd(state: GlState, args: string[]) {
  ensureHelpNotRequested(
    args,
    [
...
function cmdPinUpdate(state: GlState, args: string[]) {
  ensureHelpNotRequested(
    args,
    [
...
function cmdVerify(state: GlState, args: string[]) {
  ensureHelpNotRequested(
    args,
...
function cmdSubtract(state: GlState, args: string[]) {
...
function cmdMerge(state: GlState, args: string[]) {
  ensureHelpNotRequested(
    args,
    [
