#!/usr/bin/env node
/**
 * gl - giterloper CLI entry point.
 * Run via: node --experimental-strip-types lib/gl.ts
 */
...
function cmdGpu(state: GlState, args: string[]): void {
  ...
  const out =
    detected.mode === "cuda"
      ...
      : { gpuMode: detected.mode, reason: detected.reason };
  commandOutput(out, state.globalJson);
  if (!state.globalJson) {
    if (detected.mode === "cuda") {
      info("CUDA detected. GPU acceleration enabled.");
    } else if ("reason" in detected && detected.reason === "no-gpu") {
      info("No NVIDIA GPU detected. Using CPU mode.");
    } else if ("reason" in detected && detected.reason === "no-toolkit") {
      info("NVIDIA GPU detected but CUDA Toolkit not found. Using CPU mode.");
      info(
        "Install CUDA Toolkit from https://developer.nvidia.com/cuda-downloads and run `gl gpu` to re-detect."
      );
    }
  }
}
...
function cmdStatus(state: GlState, args: string[]): void {
  ensureHelpNotRequested(
    args,
    ["Usage: gl status [--json]", "Shows pin, clone, collection, and qmd state."].join("\n")
  );
...
function cmdPinAdd(state: GlState, args: string[]): void {
  ...
  const newPin: Pin = { name, source, sha, branch };
  ...
  commandOutput(
    { name, source, ref, branch: branch || null, sha, action: "pin-added" },
    state.globalJson
  );
}
...
function cmdPinUpdate(state: GlState, args: string[]): void {
  ensureHelpNotRequested(
    args,
    ["Usage: gl pin update <name> [--ref <ref>]", "Resolves a new SHA, clones/indexes it, and updates pin."].join(
      "\n"
    )
  );
  ...
  if (newSha.toLowerCase() === oldPin.sha.toLowerCase()) {
    commandOutput(
      { name, sha: newSha, updated: false, reason: "already at requested sha" },
      state.globalJson
    );
    return;
  }
  ...
  commandOutput(
    { name, oldSha: oldPin.sha, newSha, updated: true },
    state.globalJson
  );
}
...
function cmdTeardown(state: GlState, args: string[]): void {
  ensureHelpNotRequested(
    args,
    ["Usage: gl teardown <name>", "Tears down pin, clone, and qmd collection."].join("\n")
  );
...
function cmdPromote(state: GlState, args: string[]): void {
  ...
  commandOutput(
    { promoted: true, pin: pin.name, oldSha: pin.sha, newSha, branch: pin.branch },
    state.globalJson
  );
}
  ...
  for (const pin of pins) {
    ...
    if (collectionPresent) {
      ...
      if (status.ok) {
        ...
        vectorsOk =
          statusText.includes(collection.toLowerCase()) && !statusText.includes("vectors: 0");
      }
    }
    results.push({
      pin: pin.name,
...
function cmdAddLike(
  ...
  const helpText =
    mode === "add"
      ...
      : [
          "Usage: gl subtract [--pin <name>] [--name <name>]",
          "Reads stdin and queues content in subtracts/.",
        ].join("\n");
  ensureHelpNotRequested(args, helpText);
...
function cmdAdd(state: GlState, args: string[]): void {
  cmdAddLike(state, args, "add");
}

function cmdSubtract(state: GlState, args: string[]): void {
  cmdAddLike(state, args, "subtract");
}
  ...
  const processQueue = (queueName: string) => {
    ...
    for (const file of files) {
      ...
      for (const rawChunk of chunks) {
        ...
        if (queueName === "added") {
          ...
          const target = path.join(
            dir,
            "knowledge",
            normalizeKnowledgeRelPath(targetRel) || file
          );
          ensureDir(path.dirname(target));
          const before = existsSync(target) ? readFileSync(target, "utf8").trimEnd() : "";
          const next = [before, "", `<!-- reconciled from added/${file} -->`, "", chunk, ""]
            .filter(Boolean)
            .join("\n");
      ...
      if (
        commitIfDirty(
          dir,
          queueName === "added" ? `gl: reconcile add ${file}` : `gl: reconcile subtract ${file}`
        )
      ) {
        commits += 1;
      }
...
function cmdMerge(state: GlState, args: string[]): void {
  ...
  const remotes = run("git", ["-C", dir, "remote"])
    .split(/\r?\n/)
    .filter(Boolean);
...
/** Exported for tooling that imports this file. */
