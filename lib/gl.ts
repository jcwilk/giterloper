...
import { EXIT, GlError, fail } from "./errors.js";
import {
...
import { isBranchNotFoundError, run, runSoft } from "./run.js";
import {
...
import { resolveSha, setCloneIdentity, toRemoteUrl } from "./git.js";
import { cloneDir, ensureDir, findProjectRoot, stagedDir } from "./paths.js";
import {
...
function cmdGpu(state: GlState, args: string[]): void {
  ...
  const out = { gpuMode: detected.mode, reason: detected.mode === "cpu" ? detected.reason : undefined };
  commandOutput(out, state.globalJson);
  if (!state.globalJson) {
    if (detected.mode === "cuda") {
      info("CUDA detected. GPU acceleration enabled.");
    } else if (detected.mode === "cpu" && detected.reason === "no-gpu") {
...
function cmdPinAdd(state: GlState, args: string[]): void {
  ...
  const ref = (refParsed.found ? refParsed.value ?? undefined : undefined) ?? branch ?? "HEAD";
  const sha = resolveSha(source, ref);
  const newPin: Pin = { name, source, sha, branch: branch ?? undefined };
...
function cmdPinUpdate(state: GlState, args: string[]): void {
  ...
  const ref: string = (refParsed.found ? (refParsed.value ?? undefined) : undefined) ?? oldPin.branch ?? "HEAD";
...
function cmdSearchLike(state: GlState, mode: "search" | "query", args: string[]): void {
  ...
  const pin = resolvePin(state, pinParsed.found ? (pinParsed.value ?? undefined) : undefined);
  const collection = collectionName(pin);
  const cmdArgs = [mode, text, "-c", collection];
  if (mode === "search" && nParsed.found && nParsed.value) cmdArgs.push("-n", nParsed.value);
...
function cmdReconcile(state: GlState, args: string[]): void {
  ...
  const processQueue = (queueName: string): { files: number; commits: number } => {
...
function main(): void {
  ...
  const gpu = localConfig.gpuMode;
  state.gpuMode = typeof gpu === "string" ? gpu : null;
...
/** Exported for tooling that imports this module in tests. */
