/**
 * QMD integration: pinQmd, collectionName, indexName, collectionExists, contextExists,
 * needsEmbeddingCount, assertCollectionHealthy, cleanupQmdFiles.
 */
...
export function assertCollectionHealthy(pin: Pin, collection: string): void {
  ...
  const vectorsLine = status
    ...
    .find(
      (line) =>
        line.toLowerCase().includes(collection.toLowerCase()) &&
