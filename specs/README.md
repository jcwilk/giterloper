...
| **reconciliation** | [reconciliation.md](./reconciliation.md) | `tests/cli/` + `tests/mcp/` + relevant `tests/core/` | Shared **inbox → corpus** reconcile semantics (**`gl reconcile`**, **`giterloper_reconcile_pending`**). **CLI** / **MCP** slice specs defer here; pair **CLI help** and **MCP tool strings** with **`reconciliation.md`** (and surface specs) so user-visible text stays alignable. |
