...
This document is the **normative** product-behavior spec for **shared library logic** exercised by **`tests/core/`** (executable checks do not override this text). It describes paths, pin file handling, reconciliation helpers, read/search adapters, and external retry policy that **CLI and MCP both rely on**. Day-to-day discovery SHOULD follow **`gl` / MCP tool help** and the **CLI** and **MCP** area specs for command- and transport-specific rules; this file states cross-cutting contracts those surfaces must stay aligned with.

For the **full pin / ref / branch decision tree** (including `giterloper_pin_set` and all user-facing edge cases), authoritative prose is being consolidated into **exactly one** area spec in tracked work. This slice states only what **`tests/core/`** already encodes and what is needed to stay **non-contradictory** with sibling specs until that consolidation ships. **(Interim — pin matrix)** paragraphs below are scoped accordingly; they MUST NOT be treated as the long-term canonical home for that material.
...
**(Interim — pin matrix)** The rules above are the **minimum** shared with CLI/MCP for session vs named pins. **`updatePinSha`** and similar **internal** lifecycle helpers MAY receive the session pin’s internal name when higher layers omitted an explicit pin; that path MUST NOT be confused with **user-facing** validation that forbids registering **`_session`** as a normal named pin.
