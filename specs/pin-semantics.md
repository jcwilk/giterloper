...
This document is the **normative** product-behavior spec for **`giterloper_pin_set`** and equivalent pin configuration at any **user-facing** API boundary. Executable checks do not override this text. Library-level enforcement lives primarily under **`tests/core/`**; MCP and CLI integration tests MUST remain aligned with the same contract.

**`specs/cli.md`** describes CLI command shapes; **`specs/MCP.md`** describes MCP transport and tool inventory—both defer here for pin naming, session pin rules, and the **branch/ref** matrix. **`specs/core.md`** covers **`pinned.yaml`** format, **`resolvePin`**, and related shared state; this file is the **single normative** home for the **`giterloper_pin_set`** decision tree and the branch/ref matrix.
...
- This spec does **not** define MCP **transport** or HTTP routing; see **`specs/MCP.md`**.
- It does **not** enumerate **CLI** subcommands beyond what the surfaces table requires; see **`specs/cli.md`**.
- It does **not** define **`pinned.yaml`** on-disk format beyond what pin storage above implies; see **`specs/core.md`**.
