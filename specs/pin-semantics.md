...
This document is the **normative** product-behavior spec for **pin naming**, **`giterloper_pin_set`** (and equivalent pin configuration at any **user-facing** API boundary), the **branch/ref matrix**, session vs named pins, merge-tool exceptions, and related error codes. Executable coverage lives under **`tests/core/`** alongside [`specs/core.md`](./core.md) (paths, `pinned.yaml` storage, and other shared library contracts). **`specs/cli.md`** and **`specs/MCP.md`** defer here for this material; they **MUST NOT** restate the matrix as a second authoritative copy.
