...
**Product-behavior** tests—assertions about what the product **must** do—live under **topic trees** **`tests/<topic>/`**.

**Naming convention:** For a slice that has a dedicated topic tree, **`<topic>`** is usually the **same basename** as the slice file **`specs/<topic>.md`** (the directory **`tests/<topic>/`** matches **`specs/<topic>.md`**). **Exceptions** are normal: a single spec may **span** several trees (see **[specs/README.md](../specs/README.md)**), and **`tests/helpers/`** is **harness-only**, not a slice pair. **[specs/README.md](../specs/README.md)** remains the slice hub—which normative files exist and how slices defer to each other. **Do not** treat this README as an index that must be updated whenever a new tree or slice appears.

Do not add or keep product-behavior coverage that has **no** representation in an **applicable** area spec (new themes need spec updates in the same change set, unless the user explicitly directs a spec change separately).

**Not** every assertion maps to a single spec bullet; the rule is that the **theme** (scenario, contract, or error shape under test) is **covered or implied** somewhere in **`specs/`** for that product area so the suite does not encode silent product law. If a test disagrees with normative **`specs/`** text, treat the spec as authoritative and align **implementation and tests** to it.
...
- **`specs/`** — normative product contracts by slice; pairing with **`tests/<topic>/`** trees is by **folder topic** and **[specs/README.md](../specs/README.md)**, not by a table in this file.
...
- **`deno run -A scripts/run-tests.ts`** (and **`deno task test`**) is the full suite entrypoint. The harness:
  ...
  - **deletes `<repository-root>/.giterloper`** and **`<repository-root>/.giterloper_test`** if they exist before scheduling so session directories from earlier runs do not accumulate on disk over many suite invocations. This **repo-root** cleanup is a **hygiene** step only; it is **not** relied on for parallelism or per-case isolation (tests still use temp `cwd` and unique session ids as below). **Policy:** cleanup stays at the repo root even though the unified harness also allocates a per-run session parent under **`tests/roots/`** (next bullet)—both behaviors are intentional: root deletion curbs stale **`<projectRoot>`**-relative trees; **`GITERLOPER_MCP_TEST_SESSION_PARENT`** isolates this run’s **`.giterloper_test/**` trees under **`tests/roots/`**.
  - **Per-harness MCP session parent (`tests/roots/`):** Immediately **after** the repo-root hygiene deletion above, the harness calls **`allocateTestRunRoot(repoRoot)`** from **`scripts/test-run-roots.ts`** (while still holding the orchestrator flock). The subtree **`tests/roots/`** is **gitignored**. Allocator creates a fresh directory under **`tests/roots/giterloper-test-runs/`**, writes a one-line JSON manifest (**`.giterloper-test-run.meta.json`**) with the same shape as **`HarnessOrchestratorRecord`** in **`scripts/harness-orchestrator-lock.ts`** (owner **PID** + **starttime** fingerprint on Linux, or **`nonlinux:<pid>`** elsewhere), and returns **`{ absoluteParent }`**. **Every** worker subprocess receives the **same** value in **`GITERLOPER_MCP_TEST_SESSION_PARENT`**, merged into a **full** copy of the parent process environment (so **`PATH`**, **`HOME`**, **`GITERLOPER_GH_TOKEN`**, etc. are preserved—ticket **`git-y614`**). Before creating a run directory it runs **bounded GC** on **only** that managed prefix: child directories whose manifest parses and is **stale** per **`isHarnessOrchestratorRecordStale`** (dead **PID** or Linux fingerprint mismatch vs live **`/proc/<pid>/stat`**) are removed recursively; manifests that are missing or not parseable are **not** deleted (not provably stale). The implementation **never** signals other processes and **never** deletes outside the resolved managed directory. **GC caps** (per **`allocateTestRunRoot`** invocation): at most **`32`** removals and at most **`256`** directory entries scanned from the managed prefix—exported as **`MAX_TEST_RUN_GC_REMOVALS_PER_INVOCATION`** and **`MAX_TEST_RUN_GC_ENTRIES_SCANNED_PER_INVOCATION`** in **`scripts/test-run-roots.ts`**. **Non-Linux:** same **PID reuse** caveat as the harness orchestrator lock ( **`kill -0`** only when **`/proc`** fingerprint is unavailable). Regression coverage for run-root allocation lives under a **`tests/<topic>/`** tree alongside the harness scripts.
  ...
  - uses a **bounded worker pool** that **backfills** from the queue as subprocesses finish. **`DENO_JOBS`** sets the number of concurrent workers (default **16** if unset) for **all** discovered logical cases under **`tests/`** (every `*.test.ts` case the AST preflight finds under any **`tests/<topic>/`** tree that follows the same discovery rules).
- There is **no** phase barrier (no “all of one topic, then all of another”); scheduling is one global queue capped only by **`DENO_JOBS`**.
...
- **Test-only env — product root:** `GITERLOPER_PROJECT_ROOT` (non-empty trimmed path) sets the **product / repository root** for **`state.projectRoot`**, constitution paths, retry logs, and (in **normal** mode) session layout: **`makeState`** and **`lib/mcp-session-store.ts`** place **`.giterloper`** under that root instead of under `<cwd>/.giterloper/`. It does **not** by itself relocate **`.giterloper_test`** when MCP test mode uses a separate session parent (see **`GITERLOPER_MCP_TEST_SESSION_PARENT`** below). Used by MCP session-store tests under the **MCP** topic tree so short-TTL `scavengeStaleSessions` cases do not delete live workspace sessions while other harness cases run in parallel.
- **Test-only env — MCP test session parent:** `GITERLOPER_MCP_TEST_SESSION_PARENT` (optional; exact name) applies **only** when the process is in **MCP test mode** (`--mcp-test-mode` or in-process equivalent). When set (non-empty after trim) and valid, session trees are **`<resolved>/.giterloper_test/<sessionId>/`**; when unset in test mode, they stay under **`<projectRoot>/.giterloper_test/<sessionId>/`** (same **`projectRoot`** as **`GITERLOPER_PROJECT_ROOT`** / **`cwd`** — see **`specs/core.md`**). **Validation** (unsafe paths, relative resolution) is defined there and enforced in **`lib/session-layout.ts`**. Long-lived editor MCP servers are unchanged unless operators set this variable. The **canonical unified harness** is expected to set **only an absolute** path here for all workers in a run (ticket **`git-y614`**) using a per-invocation directory under **`tests/roots/`** (allocation + GC: ticket **`git-jp1p`**). Per-topic **`deno task test:*`** entrypoints use **`scripts/deno-test-topic.ts`** to set the same variable for a **single** child **`deno test`** process (see **Topic tasks and manual `deno test` (bypass)** below). **True bypass** (raw **`deno test`**, IDE-run single files, etc.) leaves it unset unless you **export** it or invoke the wrapper yourself.
- Each logical test case uses a shared **test runtime context**: unique **`sessionId`**, unique **`runId`**, dedicated **`cwd`** (typically a temp directory), and **injected** MCP/server/CLI configuration. **Normal-mode** session state would live under **`<projectRoot>/.giterloper/<sessionId>/`**; integration helpers default to MCP test mode, so persisted session dirs are under **`<sessionsParent>/.giterloper_test/<sessionId>/`** (see **`GITERLOPER_MCP_TEST_SESSION_PARENT`** above).
...
- **CLI and gl-maintenance tests** must not rely on the implicit `_cli` session for isolation. Use **`TestRuntimeContext`** from `tests/helpers/test-runtime-context.ts`: `createTestRuntimeContext()` yields a temp **`cwd`**, unique **`sessionId`**, and **`runId`** (for pin/branch/file names). Pass **`{ ctx }`** into `runGl` / `runGlJson` / `runGlMaintenance` / `runGlMaintenanceJson` from `tests/helpers/gl.ts`, or pass explicit **`cwd`** + **`sessionId`**—helpers do **not** default subprocess `cwd` to the repo root. **`runGl` / `runGlMaintenance`** pass **`--mcp-test-mode`** and merge **`integrationMcpModeChildEnv()`** from `tests/helpers/integration-mcp-env.ts` (sets **`TEST_KNOWLEDGE_STORE_REMOTE`**) so CLI subprocesses that write session state use **`.giterloper_test/<sessionId>/`** under the effective session parent (default **`<projectRoot>/.giterloper_test/`** per **`specs/core.md`**; the unified harness may set **`GITERLOPER_MCP_TEST_SESSION_PARENT`**) and the shared test knowledge remote, not **`.giterloper/`** with production-style configuration. Use **`scratchPinName(ctx, prefix)`** for scratch pins; tear down with **`destroyTestRuntimeContext(ctx)`** (often from an **`unload`** listener on the context created for that file or case). For **`cleanupTestKnowledgeRepo`**, when **`pinName`** + **`sessionId`** are set, pass **`cwd`** as the directory that **contains** **`.giterloper_test/`** for that run (the effective **`sessionsParent`** / subprocess **`projectRoot`** per **`specs/core.md`**—usually **`ctx.cwd`** when helpers run **`gl`** with that cwd and no **`GITERLOPER_PROJECT_ROOT`** override; use the pinned product root or **`GITERLOPER_MCP_TEST_SESSION_PARENT`** target when those relocate session layout).
- **MCP tests** must not use **`Deno.env.set` / `delete`** to configure auth, insecure mode, or knowledge-store bootstrap. Pass explicit config into server/app constructors and test factories (see seams in `lib/gl-mcp-server.ts`, `lib/mcp-auth.ts`, `lib/gl-core.ts`, `lib/mcp-session-store.ts`). Production entrypoints may still read env **once** at startup; tests inject config objects instead of mutating process-global env. **`createMcpAppForTest`** defaults **`mcpTestMode`** to **`true`** when omitted so in-process MCP integration tests use **`.giterloper_test`** and **`TEST_KNOWLEDGE_STORE_REMOTE`** semantics unless a case explicitly passes **`mcpTestMode: false`**. Pair **`mcpTestMode: true`** with **`knowledgeStoreRemote`** (or rely on **`TEST_KNOWLEDGE_STORE_REMOTE`** in env) per **`specs/mcp.md`**. For parallel in-process servers in one process, pass **`projectRoot`** and/or **`mcpTestSessionParent`** on **`createMcpAppForTest`** (**`createServer`** options): with **`mcpTestMode: true`**, a non-empty **`projectRoot`** pins both product root and the default **`.giterloper_test`** tree under that directory and **ignores** **`GITERLOPER_MCP_TEST_SESSION_PARENT`** unless **`mcpTestSessionParent`** is also set. An explicit **`mcpTestSessionParent`** overrides the env var for that instance (**`specs/core.md`**).
...
- **`--mcp-test-mode`** — on **`gl`**, **`gl-maintenance`**, **`lib/gl-mcp-server.ts`**, and **`lib/gl-mcp-server-stdio.ts`**, selects MCP test mode: session state uses the literal **`.giterloper_test`** base under **`sessionsParent`** (**`<projectRoot>/.giterloper_test/`** by default; optional **`GITERLOPER_MCP_TEST_SESSION_PARENT`** overrides the parent directory only — **`specs/core.md`**). **`GITERLOPER_PROJECT_ROOT`** still sets **`projectRoot`** for constitution and **`state.projectRoot`**; it does not replace the session-parent override. The effective knowledge remote is read from **`TEST_KNOWLEDGE_STORE_REMOTE`** (unless overridden in-process). There is **no** env var that toggles this mode.
...
| HTTP server consumers | Tests under the **MCP** topic tree that call **`spawnMcpHttpIntegrationServer`** | Call **`spawnMcpHttpIntegrationServer`** | **`server?.kill()`** in **`finally`** after closing MCP clients. |
| Stdio smoke (with memsearch wrap) | **`spawnMcpStdioIntegrationServer`** — `tests/helpers/mcp-subprocess.ts`; consumer tests under the **MCP** topic tree | **`node:child_process` `spawn`**: outer Deno → **`with-memsearch`** → inner Deno → stdio MCP (piped stdin/stdout) | Same as HTTP: **`detached: true`** (Unix) + **`kill()`** on the handle uses **`process.kill(-pid, "SIGTERM")`** for the **process group**; **`stdin.end()`**, **`stdout.destroy()`**, then **`await once(proc, "exit")`**. |
| Startup / validation (no `with-memsearch`) | Tests under the **MCP** topic tree that exercise remote startup (e.g. via **`runEntrypoint`**) | **`Deno.Command`** `deno run -A <script>` — **one** Deno per invocation, short-lived **`output()`** | Process exits when the entrypoint exits; many invocations per file, different shape from integration servers. |
...
| CLI under test mode | **`runGl` / `runGlMaintenance`** — `tests/helpers/gl.ts` | Subprocess **`gl`** / **`gl-maintenance`** with **`--mcp-test-mode`** | Single child per invocation; uses **`.giterloper_test`** under the effective session parent (see **`GITERLOPER_MCP_TEST_SESSION_PARENT`**); not the same as spawning **`gl-mcp-server.ts`**. |
...
**Source anchors (file:line — may drift on edit):** **`tests/helpers/mcp-subprocess.ts`** (spawn helpers and teardown); **`scripts/with-memsearch.ts`** (inner **`Deno.Command`**); consumer tests under **`tests/<topic>/`** for the **MCP** slice; **`tests/helpers/gl.ts`** (`runGl` / `runGlMaintenance`, **`--mcp-test-mode`**).
...
Tests under topic trees whose slices define **user-facing** surfaces (**CLI**, **MCP**, and similar) are especially valuable as executable workflow documentation: they show what correct behavior looks like in an unambiguous way that both humans and agents can follow.

## Integration scope: less is more

- Keep integration-style workflow coverage focused on core paths and contract boundaries.
...
- Use the **`core`** slice’s topic tree (see **[specs/README.md](../specs/README.md)**) for combinatorial edge cases and implementation-level details.
...
- Tests under the **MCP** slice’s topic tree that exercise search—hosts must end up with **`memsearch` on `PATH`**; the default **`deno task`** / **`check_all`** paths arrange that automatically.
...
### MCP topic tree and server factories

Normative **product** rules for MCP startup (including **memsearch** on **`PATH`**) live in **[specs/mcp.md](../specs/mcp.md)**. This subsection is **harness-only**: how the **MCP** topic tree and test factories construct servers without weakening production entrypoints.

- Cases under that tree that exercise **`giterloper_search`** MUST **not** be marked **ignored**, **skipped**, or otherwise bypassed solely because **memsearch** is missing from the environment. Those tests assume a correctly provisioned host (including **memsearch** on **`PATH`**), consistent with the MCP contract. The default **`deno task test`** / **`check_all`** paths bootstrap **memsearch** when absent (see Memsearch above).
...
- All discovered product-behavior trees under **`tests/`** follow the **same** isolation rules: no reliance on shared repo-root **`.giterloper/`** or **`.giterloper_test/`** or mutable **`Deno.env`** between concurrent cases (for example subprocess **`gl`** helpers with **`TestRuntimeContext`** plus **`integrationMcpModeChildEnv`**; in-process MCP with **`createMcpAppForTest`** and default test mode).

### Layout and individual commands

Tests are grouped by **topic** under **`tests/<topic>/`**, not by duration. **Role of a tree** matches its name and the slice(s) it targets; see **[specs/README.md](../specs/README.md)** for slice contracts. This README does **not** maintain a per-directory index—add new topic folders as needed without updating this section for mapping.
...
- **Topic only:** **`deno task test:<topic>`** when defined in **`deno.json`** — runs **`deno test`** against **`tests/<topic>/`** for fast feedback; same isolation expectations as the full suite. Task names and which topics exist are **source-defined** in **`deno.json`** (not listed here).

### Topic tasks and manual `deno test` (bypass)

Topic **`deno task`** entrypoints **do not** acquire the harness orchestrator flock; they run one **`deno test`** child. To align MCP test-mode session layout with the unified harness (per-run parent under **`tests/roots/giterloper-test-runs/`** instead of repo-root **`.giterloper_test/`**), most topic tasks use **`deno run -A scripts/deno-test-topic.ts -- test …`**, which calls **`allocateTestRunRoot`** from **`scripts/test-run-roots.ts`** and sets **`GITERLOPER_MCP_TEST_SESSION_PARENT`** (exact name from **`lib/session-layout.ts`**) on the child’s environment (full parent **`env`** merge, same spirit as ticket **`git-y614`**). Individual tasks may prepend **`with-memsearch`** (or similar) so **memsearch** stays on **`PATH`** where required—see **`deno.json`** for the exact graph per **`test:<topic>`**.

**Manual raw `deno test`** (single file, debugger, or any path that skips **`run-tests.ts`** and the topic wrapper): in MCP test mode, unset **`GITERLOPER_MCP_TEST_SESSION_PARENT`** means sessions use **`<projectRoot>/.giterloper_test/<sessionId>/`**. To match harness/topic behavior, either run **`deno run -A scripts/deno-test-topic.ts -- test …`** from the repo root, or **export** an **absolute** directory path (create it first if needed), e.g. `export GITERLOPER_MCP_TEST_SESSION_PARENT="$(realpath ./path/to/parent)"` then **`deno test …`**. Relative values are resolved per **`specs/core.md`** / **`lib/session-layout.ts`**; prefer **absolute** paths so layout does not depend on each process’s **`cwd`**. Reconciliation OpenAI VCR tests under **`tests/reconciliation/`** force **`GITERLOPER_OPENAI_VCR=replay-only`** whenever the effective mode would otherwise be **`off`** (variable unset, explicit **`off`**, or any unknown value) so committed tapes still run and the case does not no-op (see **`lib/reconcile-openai-vcr.ts`** for **`rerecord-all`** and concurrent top-level **`deno test`** caveats).

## CLI / MCP integration tests: collision avoidance (CRITICAL)

Tests that hit `giterloper_test_knowledge` use a shared remote repository. Local state must remain **per session** and **per test case** under **`<sessionsParent>/.giterloper_test/<sessionId>/`** when using the integration helpers (MCP test mode); **`sessionsParent`** is usually **`<projectRoot>`** from the test **`cwd`** / env unless the harness sets **`GITERLOPER_MCP_TEST_SESSION_PARENT`** (**`specs/core.md`**).
...
- Each logical case has its own `sessionId` and cwd; state lives under **`.giterloper_test/<sessionId>/`** beneath the effective **`sessionsParent`** when helpers set MCP test mode (default for CLI/MCP integration paths documented above; **`sessionsParent`** defaults to **`projectRoot`**, not always the case’s **`cwd`**).
