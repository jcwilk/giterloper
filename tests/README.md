...
Topic **`deno task`** entrypoints **do not** acquire the harness orchestrator flock; they run one **`deno test`** child. To align MCP test-mode session layout with the unified harness (per-run parent under **`tests/roots/giterloper-test-runs/`** instead of repo-root **`.giterloper_test/`**), **`deno.json`** wires **`test:core`**, **`test:cli`**, and **`test:mcp`** through **`deno run -A scripts/deno-test-topic.ts -- test …`**, which calls **`allocateTestRunRoot`** from **`scripts/test-run-roots.ts`** and sets **`GITERLOPER_MCP_TEST_SESSION_PARENT`** (exact name from **`lib/session-layout.ts`**) on the child’s environment (full parent **`env`** merge, same spirit as ticket **`git-y614`**). **`test:pin-semantics`** chains **`scripts/with-memsearch.ts`** → **`deno-test-topic.ts`** → **`deno test`** so **memsearch** stays on **`PATH`**.
