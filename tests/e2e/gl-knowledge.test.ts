...
Deno.test("gl-knowledge e2e", async (t) => {
  ...
  await t.step("diagnostic reports healthy state (main)", () => {
    const result = runGlJson(["diagnostic", "--pin", TEST_PIN_NAME]) as {
      ok?: boolean;
      pins?: { pin?: string; ok?: boolean; clonePresent?: boolean; cloneShaOk?: boolean; collectionPresent?: boolean; vectorsOk?: boolean }[];
    };
    assertEquals(result.ok, true, "diagnostic should report ok=true");
    assertEquals((result.pins?.length ?? 0) > 0, true, "diagnostic should include pins");
    const pinCheck = result.pins!.find((p) => p.pin === TEST_PIN_NAME);
    assertExists(pinCheck, "diagnostic should include test pin");
    assertEquals(pinCheck!.ok, true, "test pin should be healthy");
  });
  ...
  await t.step("teardown", async () => {
    try {
      runGlExtendedJson(["teardown", TEST_PIN_NAME]);
    } finally {
      cleanupTestKnowledgeRepo(TEST_SOURCE, CLEAN_MAIN_SHA, {
        pinName: TEST_PIN_NAME,
        branchName: WORKFLOW_BRANCH,
