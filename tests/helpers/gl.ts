...
/**
 * Runs gl-extended commands (status, verify, gpu, pin remove, clone, index, teardown, stage, stage-cleanup).
 * Use for tests that need debugging/maintenance commands not in main gl.
 */
export function runGlExtended(
  ...
  let result;
  if (opts.stdin != null && opts.stdin !== "") {
    const tmp = mkdtempSync(path.join(tmpdir(), "gl-ext-stdin-"));
    const stdinFile = path.join(tmp, "stdin.txt");
    try {
      writeFileSync(stdinFile, opts.stdin, "utf8");
      result = spawnSync("sh", ["-c", `"${GL_EXTENDED_SCRIPT}" ${cliArgs.map((a) => `'${a.replace(/'/g, "'\\''")}'`).join(" ")} < ${stdinFile}`], {
        cwd,
        encoding: "utf8",
        stdio: ["pipe", "pipe", "pipe"],
      });
    ...
    result = spawnSync(GL_EXTENDED_SCRIPT, cliArgs, {
      cwd,
      encoding: "utf8",
      stdio: ["pipe", "pipe", "pipe"],
    });
